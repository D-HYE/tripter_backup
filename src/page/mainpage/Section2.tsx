...
export default function Section2(){
    ...
    return(
        <section className="section section2">
            <h3 className="section_tit py-20 font-theJamsil-bold text-mediumbig-text text-center">
                MD 추천 테마여행지
            </h3>
            <div >
                ...
                >
                    {slides.map((item ,idx) => (
                        ...
                        >
                            <Link to={item.link ?? '/'}>
                                <div className="img_box h-[10rem] md:h-[15rem] rounded-[20px] overflow-hidden">
                                    <img src={item.src} alt={item.alt} className='w-full h-full object-cover'/>
                                </div>
                                <div className="swiper_desc">
                                    <b className="items-center justify-center flex-col h-[4.5rem] py-3 text-semismall-text hidden md:flex">{item.title}{item.title_span && <span className='text-semismall-text'>{item.title_span}</span>}</b>
